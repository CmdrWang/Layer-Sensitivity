...
def prune_wanda_outlier_structure_special(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, all_layer_ratio=None):
    ##### calucalte outlier ratio
    all_layer_ratio=[]
    use_cache = model.config.use_cache 
    model.config.use_cache = False
    # --- SparAlloc per-layer sparsity support ---
    num_layers = len(model.model.layers) if hasattr(model, 'model') else len(model.layers)
    if all_layer_ratio is None:
        # 默认：与旧版本完全一致（全局一个 sparsity）
        all_layer_ratio = np.array([args.sparsity_ratio] * num_layers, dtype=float)
    else:
        all_layer_ratio = np.array(all_layer_ratio, dtype=float)

    all_layer_ratio = np.clip(all_layer_ratio, 0.0, 1.0)
# --------------------------------------------
...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, all_layer_ratio=None):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    ...
    for i in range(len(layers)):
        layer = layers[i]
        subset = find_layers(layer)
        layer_sparsity_ratio = float(all_layer_ratio[i])  # 本层要剪掉的比例
...
def prune_wanda_outlier_structure(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, all_layer_ratio=None):
    ##### calucalte outlier ratio
    all_layer_ratio=[]
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
...
def prune_wanda_outlier(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, all_layer_ratio=None):
    ##### calucalte outlier ratio
